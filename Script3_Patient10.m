% This is script which runs the pipeline from an existing cut biventricular surface mesh to
% simulation files
...
mesh_resolution_fine=1.2;
mesh_resolution_coarse=1.5;
mesh_resolution_hexa=0.05;
                                 
%% get files name
cd(origpath)
name_origin='Patient_10_refined_surface_mesh_aligned.ply'; %name of the original surface
name_final='Patient_10_refined_surface_mesh_aligned'; %name of the final mesh and folder
fibrosis_filename='C:\Users\arant\Documents\MEGA\Postdoc\CNPq\Conhecimento_Brasil_2024\Programas\InSilicoHeartGen-main\inputs\Patient_10_refined_fibrosis.vtu';   %name of the fibrosis region file
...
for index=1
    ...
    %% read original mesh
        ...
        Field_generator_with_fibrosis_UKBB_function24(Fiber_info,meshformat,pericardium_level, epiendo, epiendoRV,requiresInterpolation,['pac10'],fibrosis_filename);
        ...
        Data4EM=load(fullfile(case_folder, strcat('ensi_Fine_','pac10'), 'Case_Fine.mat')) ; %choose between the mesh files  (coarse for EM or fine for EP)
...
%         %read field data
        casepath=fullfile(resultspath,name_final,'ensipac10');
...
fibrosis_filename = strcat(fibrosis_dir,'\ensipac10\CSVFiles\pac10_refined_fibrosis_mask.csv');
